...
corr_weight <- 0.5
mag_weight <- 0.5
score_nSales <- merge(corr_nSales, magnitude_nSales, by = c("Store1", "Store2") )[, scoreNSales := corr_weight* corr_measure + mag_weight *mag_measure]
score_nCustomers <- merge(corr_nCustomers, magnitude_nCustomers, by = c("Store1", "Store2") )[, scoreNCust := corr_weight* corr_measure + mag_weight *mag_measure ]
print(score_nSales)
```
...
percentageDiff <- merge(scaledControlSales[, c("YEARMONTH", "controlSales")],
                        ...
                        by = "YEARMONTH")[, percentageDiff := abs(controlSales-totSales)/controlSales][order(-percentageDiff)]
```
